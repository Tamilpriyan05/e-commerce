...
import { Jbldata } from "./Jbldata";

class Jbl extends React.Component {
  constructor(props) {
    super(props);
  ...
  tocart = () => {
    ...
    let popupj = this.state.Arryobj.map((val) => {
      return (
        <>
          <div>
            <img src={val.img}></img>
    ...
    if (sum > 0) {
      this.props.topopupj(popupj);
      this.props.toheader(sum);
    }
  };
  render() {
    let jblcard = this.state.Arryobj.map((val) => {
      return (
        <>
        ...
        <div className="container">
          <div className="box">
            <div className="pagelogo">
              <img src={jbllogo} height={200} width={300}></img>
            </div>
            <div className="flex allcard">{jblcard}</div>
          </div>
        </div>
      </>
    );
